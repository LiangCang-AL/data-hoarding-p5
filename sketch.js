...
// ============================================================
// 2. Chat History：麦田拖拽割麦（原 450×600 竖版坐标映射）
// ============================================================
class WheatScene extends BaseScene {
  ...
  updateTransform() {
    this.scale = min(this.app.w / this.W, this.app.h / this.H);
    this.offsetX = (this.app.w - this.W * this.scale) / 2;
    this.offsetY = (this.app.h - this.H * this.scale) / 2;
  }
  ...
  drawSideMask() {
    ...
    stroke(255, 18);
    strokeWeight(1);
    rect(this.offsetX, this.offsetY, this.W * this.scale, this.H * this.scale);
    pop();
  }
  ...
  createPerspectiveField() {
    ...
    for (let r = 0; r < C.FIELD_ROWS; r++) {
      ...
      const count = floor(lerp(7, 25, t));
      const scaleV = lerp(0.45, 1.15, t);
  ...
  pickOriginalDistributionPoint() {
    ...
    for (const s of this.stalks) {
      if (s.slotId !== undefined && s.slotId !== null) occupied.add(s.slotId);
    }
...
class WheatStalk {
  ...
  drawStem() {
    ...
    if (this.scene.C.STEM_STYLE === "line") this.drawStemLine(visibleHeight, 180);
    else if (this.scene.C.STEM_STYLE === "letters") this.drawStemLetters(visibleHeight, 165);
    else this.drawStemBlocks(visibleHeight, 165);
  }
  ...
  getStemPointAt(t) {
    t = constrain(t, 0, 1);
    const sway = sin(frameCount * this.swaySpeed + this.noiseSeed + t * 1.8) * this.swayAmp * 0.42 * (0.25 + t);
    const noiseSway = (noise(this.noiseSeed + t * 2.3, frameCount * 0.01) - 0.5) * this.swayAmp * 0.55 * t;
    return {
  ...
  getVisibleLetterPositions() {
    ...
    for (let i = 0; i < this.earCount; i++) {
      ...
      const level = i / max(1, this.earCount - 1);
      const microSway = sin(frameCount * this.swaySpeed + this.noiseSeed + i * 0.35) * this.swayAmp * 0.16 * (1 - level);
      ...
      arr.push({ x, y, char: this.letters[i], index: i, size, appearAlpha, level });
    }
    return arr;
  }
...
class WheatCutStem {
  ...
  getStemPointAt(t) {
    t = constrain(t, 0, 1);
    const sway = sin(frameCount * this.swaySpeed + this.noiseSeed + t * 1.8) * this.swayAmp * 0.35 * (0.25 + t);
    const noiseSway = (noise(this.noiseSeed + t * 2.3, frameCount * 0.01) - 0.5) * this.swayAmp * 0.45 * t;
    return {
      x: this.baseX + sway + noiseSway,
      y: this.baseY - t * this.stemLength
    };
...
class WheatFallenEar {
  constructor(scene, stalk, positions, cutX, cutY) {
    ...
    for (const p of positions) {
      ...
      const targetY = heapBaseY - layer * C.HEAP_LAYER_STEP * stalk.scale + random(-C.HEAP_SETTLE_JITTER, C.HEAP_SETTLE_JITTER) * stalk.scale;
      const cutPushX = (p.x - cutX) * 0.01;
      const cutPushY = (p.y - cutY) * 0.006;

      this.parts.push(new WheatFallenLetter(scene, p.x, p.y, targetX, targetY, p.char, p.size, p.level, stalk.scale, cutPushX, cutPushY));
    }
  }
  ...
  display() {
    for (const p of this.parts) p.display();
  }
}
...
class WheatFallenLetter {
  ...
  update() {
    ...
    if (!this.landed) {
      ...
      if (this.landAge > C.FALL_FADE_DELAY) this.alpha -= C.FALL_FADE_SPEED;
      if (this.alpha <= 0) this.dead = true;
    }
  }
